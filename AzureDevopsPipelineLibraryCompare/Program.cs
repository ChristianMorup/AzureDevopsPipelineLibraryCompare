using AzureDevopsPipelineLibraryCompare;
using Spectre.Console;
using Environment = AzureDevopsPipelineLibraryCompare.Environment;

Console.Clear();

var devOpsService = new AzureDevOpsService();

var azureDevopsServices = Environment.Instance.GetAllAzureDevopsServices();

if (azureDevopsServices.Count == 0)
    Environment.Instance.RequestUserForNewAzureDevopsService();
else
{
    var addNewServiceOption = "Add a new service...";
    azureDevopsServices.Add(addNewServiceOption);

    var chosenService = AnsiConsole.Prompt(
        new SelectionPrompt<string>()
            .Title("What Azure Devops service do you wish to use?")
            .PageSize(10)
            .MoreChoicesText("[grey](Move up and down to reveal more services)[/]")
            .AddChoices(azureDevopsServices));

    if (chosenService == addNewServiceOption)
        Environment.Instance.RequestUserForNewAzureDevopsService();
    else
        Environment.Instance.SetCurrentAzureDevOpsInstance(chosenService);
